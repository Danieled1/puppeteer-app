...
module.exports = async function gradesFlow(page, context = {}) {
  ...
  try {
    ...
    console.log(`🕒 gradesFlow completed in ${totalTime}ms`);

    if (totalTime > 7000) {
      console.warn(`⚠️ SLOW PAGE: grades page took ${totalTime}ms to fully render`);
    }
    if (context.shouldExport) {
      addMetric({
        ...
        tableRowMs: Math.round(timings.rowLoad),
        rowCount,
