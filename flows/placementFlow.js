  ...
  module.exports = async function placementFlow(page) {
    ...
    try {
      ...
      page.on('requestfinished', async request => {
        ...
        if (url.includes('admin-ajax.php') || url.includes('/wp-json/')) {
          try {
            ...
            const duration = timing && timing.receiveHeadersEnd && timing.startTime
              ? timing.receiveHeadersEnd - timing.startTime
              : -1;
      ...
      if (courseItems.length > 0) {
        ...
        console.warn('❌ No related job-prep courses found');
      }
